...
const SportsBar : React.FC<Sports_>= ({sports}) => {
    ...
    const [price, setPrice] = useState(sports.priceRanges[0].max);
  
    const ticketPrices: { Standard: number; VIP: number; VVIP: number } = {
        Standard:sports.priceRanges[0].max ,
        VIP: sports.priceRanges[0].max*1.5,
        VVIP:sports.priceRanges[0].max*2,
      };
